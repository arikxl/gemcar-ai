    ...
    <div className="pt-20 flex flex-col">
      ...
      <section className="relative py-16 md:py-28 dotted-bg">
        <div className="max-w-4xl mx-auto text-center">
          <div className="mb-8">
            <h1 className="gradient-title text-5xl md:text-8xl mb-4">Find your Dream Car with GemCar AI</h1>
      ...
      <section className="py-12 bg-gray-100">
        <div className="px-4 container mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
            {carMakes.map((make) => (
              <Link key={make.id} href={`/cars?make=${make.name}`}
      ...
      <section className="py-12 bg-gray-100">
        <div className="px-4 container mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4  gap-4">
            {bodyTypes.map((body) => (
              <Link key={body.id} href={`/cars?bodyType=${body.name}`}
                className="relative group cursor-pointer">
                <div className="overflow-hidden rounded-lg flex justify-end h-28 mb-4 relative">
                  <Image src={body.image} alt={body.name} width={250} height={100}
                    className="object-cover group-hover:scale-105 transition duration-300 text-center mx-auto" />
                </div>
                <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent rounded-lg flex items-end">
                  <h3 className="text-white text-xl font-bold pl-4 pb-2">{body.name}</h3>
                </div>
              </Link>
            ))}
          </div>
